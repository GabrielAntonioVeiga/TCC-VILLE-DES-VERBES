...
if (collision_circle(x, y, global.range, Player, false, true)) {
    ...
    // 🆕 ADICIONA AQUI — concede o quadro da missão
    if (!variable_global_exists("quadros_coletados")) {
        global.quadros_coletados = {};
    }
    if (!variable_struct_exists(global.quadros_coletados, missao_id)
        || global.quadros_coletados[$ missao_id] == false) {
        global.quadros_coletados[$ missao_id] = true;
        var _popup = instance_create_depth(0, 0, -9999, obj_popup_quadro);
        _popup.sala_id = missao_id;
    }
