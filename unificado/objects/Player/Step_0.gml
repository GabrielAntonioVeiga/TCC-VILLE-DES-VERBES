...
if (distance_to_object(ParenteNpc) <= global.range) {
    if (keyboard_check_pressed(ord("E")) && global.dialogo == false && !instance_exists(Dialogo)) {
        ...
        // Evita que o Player crie um diálogo duplo se o NPC de missão já estiver fazendo isso
        if (npc.object_index != obj_npc_missao) {
            
            var _dialogoInst = instance_create_layer(x, y, "Dialogo", Dialogo);
            
            // BLINDAGEM ANTI-CRASH: Só puxa o nome se a variável existir no objeto
            if (variable_instance_exists(npc, "nome")) {
                _dialogoInst.nomeNpc = npc.nome;
            } else {
                _dialogoInst.nomeNpc = ""; // Deixa em branco caso seja só uma mobília
            }
...
if (distance_to_object(ObjObstaculoQuiz) <= global.range) {
    ...
    // Checa o clique do "E" usando as proteções certas
    if (keyboard_check_released(ord("E")) && global.dialogo == false && !instance_exists(obj_quiz) && !instance_exists(Dialogo)) {
        
        // 1. Cria o objeto do quiz na tela
        var _q = instance_create_layer(0, 0, "interacao", obj_quiz);
        
        // 2. Passa adiante a variável correta que configuramos no Creation Code da Room
        if (variable_instance_exists(_instancia_perto, "meu_quiz_id")) {
            _q.pergunta_id = _instancia_perto.meu_quiz_id; 
        }
        
        // 3. Trava o jogador para o diálogo
        global.dialogo = true; 
    }
}
